  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
        ...
        <div class="space-y-6">
          <div class="bg-bg-tertiary border border-border-primary rounded-xl overflow-hidden">
            <!-- Design tabs -->
            <div class="flex bg-bg-quaternary border-b border-border-primary">
              <button
                ...
                :class="[
                  ...
                  activeDesignTab === tab
                    ? 'text-primary bg-bg-tertiary border-primary'
            ...
            <div class="p-8 min-h-[600px] flex items-center justify-center">
              <div class="w-full max-w-4xl">
                <!-- Mockup container -->
                <div class="bg-bg-quaternary border border-border-primary rounded-lg overflow-hidden shadow-lg">
                  <div class="bg-bg-secondary px-4 py-3 flex items-center gap-2 border-b border-border-primary">
                  ...
                  <div class="p-8 text-center">
                    ...
                    <div v-if="activeDesignTab === 'Desktop'" class="space-y-6">
                      <div class="bg-bg-secondary border-2 border-dashed border-border-secondary rounded-lg p-16">
                      ...
                      <div class="grid grid-cols-3 gap-4">
                        <div v-for="i in 3" :key="i" class="bg-bg-secondary border border-border-primary rounded-md h-32"></div>
                      </div>
                    </div>
                    
                    <div v-else-if="activeDesignTab === 'Tablet'" class="space-y-6">
                      <div class="bg-bg-secondary border-2 border-dashed border-border-secondary rounded-lg p-12">
                        <p class="text-text-tertiary">Tablet View</p>
                      </div>
                      <div class="grid grid-cols-2 gap-4">
                        <div v-for="i in 2" :key="i" class="bg-bg-secondary border border-border-primary rounded-md h-32"></div>
                      </div>
                    </div>
                    
                    <div v-else-if="activeDesignTab === 'Mobile'" class="max-w-sm mx-auto space-y-4">
                      <div class="bg-bg-secondary border-2 border-dashed border-border-secondary rounded-lg p-8">
                        <p class="text-text-tertiary">Mobile View</p>
                      </div>
                      <div class="space-y-3">
                        <div v-for="i in 3" :key="i" class="bg-bg-secondary border border-border-primary rounded-md h-24"></div>
                      </div>
                    </div>
                    ...
                    <div v-else class="space-y-4">
                      ...
                      <div class="grid grid-cols-2 gap-4">
                        <div v-for="i in 4" :key="i" class="bg-bg-secondary border border-border-primary rounded-md h-24 p-4 flex items-center justify-center text-text-muted text-sm">
                          Component {{ i }}
                        </div>
                      </div>
                    </div>
        ...
        <div class="space-y-6">
          <!-- Overview -->
          <div class="bg-bg-tertiary border border-border-primary rounded-xl p-6">
            ...
            <div class="pt-4 border-t border-border-primary">
              ...
              <div class="h-1.5 bg-bg-quaternary rounded-full overflow-hidden">
                <div 
          ...
          <div class="bg-bg-tertiary border border-border-primary rounded-xl p-6">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              <span 
                ...
                :key="tag"
                class="px-3 py-1 bg-bg-quaternary border border-border-primary rounded-xl text-xs text-text-secondary"
              >
                {{ tag }}
              </span>
            </div>
...
const emit = defineEmits(['back', 'update-context'])
...
const loadPageData = () => {
  ...
  const pages = storage.getPages(props.projectId)
  page.value = pages.find(p => p.id === props.pageId)
  ...
  // Update context when page is loaded
  emit('update-context', {
    type: 'page',
    item: page.value
  })
...
const saveSectionsToStorage = () => {
  ...
  storage.savePage(props.projectId, page.value)
}
